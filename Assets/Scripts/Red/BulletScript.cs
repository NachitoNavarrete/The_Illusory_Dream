...
public class Bullet : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (red != null)
        {
            ...
            if (red.IsAlive)
            {
                ...
                else
                {
                    ...
                    if (!red.IsInvulnerable && !red.IsAdminModeActive)
                    {
        ...
        // Nota: no dependemos del tag "Player" (Red puede estar Untagged); basta con
        // que el propietario tenga RedMovement con el Modo Admin activo.
        ...
        var rBoss = collision.GetComponent<RobotBoss>();
        if (rBoss != null)
        {
            rBoss.TakeDamage(finalDamage);
            DestroyBullet();
            return;
        }
