...
/* PollenEffect.cs: efecto de polen que aplica daño con el tiempo y ralentiza enemigos. */
using UnityEngine;
...
public class PollenEffect : MonoBehaviour
{
    ...
    private void Start()
    {
        // CrowBoss y RobotEnemy son inmunes a este efecto; Grunt también
        if (GetComponent<CrowBoss>() != null || GetComponent<RobotEnemy>() != null || GetComponent<GruntEnemy>() != null)
        {
            Destroy(this);
            return;
        }
        ...
        // Generar partículas moradas alrededor del enemigo
        SpawnPurpleParticles();

        // Iniciar daño por tiempo (DOT)
