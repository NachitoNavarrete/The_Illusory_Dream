...
public class HealthPickup : MonoBehaviour
{
    ...
    [Tooltip("Sonido que se reproduce al recoger la vida.")]
    public AudioClip pickupSound;

    private void Start()
    {
        // La vida que spawnea debe tener exactamente 5 segundos para desaparecer
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        // Evitar m�ltiples activaciones si el jugador tiene varios colliders
        Collider2D selfCol = GetComponent<Collider2D>();
        if (selfCol != null) selfCol.enabled = false;

        // Usar el m�todo Heal del jugador (aplica clamp a MaxHealth)
        red.Heal(HealAmount);
