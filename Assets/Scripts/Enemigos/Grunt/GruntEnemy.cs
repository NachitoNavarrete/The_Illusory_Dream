...
public class GruntEnemy : MonoBehaviour
{
    ...
    [Header("Visual Scale")]
    public float scaleMultiplier = 1.5f; // Increased from 0.65f for larger grunts
    ...
    [Header("Loot")]
    public System.Collections.Generic.List<Loot> lootTable = new System.Collections.Generic.List<Loot>();
    ...
    private void Start()
    {
        ...
        /*
         ...
         - Esto se ejecuta cuando aparece el enemigo en la escena por primera vez.
         - "Cachear componentes" significa que guarda referencias a sus partes (como
           el cuerpo físico, la imagen y las animaciones) para no buscarlas muchas veces.
         - También busca al jugador para saber a quién seguir o a quién disparar.
         - Si faltan cosas (como la bala o el sonido), intenta tomarlas del jugador.
        */
        ...
        // Get FireSound from player if not assigned
        if (FireSound == null && Red != null)
        {
            var soundCtrl = Red.GetComponentInChildren<RedSoundController>();
            if (soundCtrl != null)
            {
                FireSound = soundCtrl.sonidoDisparo;
            }
        }
    }
    ...
    private void Update()
    {
        ...
        /*
         ...
         - Decide qué hacer ahora: mirar al jugador, moverse hacia él, disparar en ráfaga
           o saltar para evitar obstáculos.
         - Usa tiempos de espera para no disparar o saltar todo el rato.
        */
        if (isDead) return;
    ...
    private bool IsGroundedGrunt()
    {
        ...
        /*
         Explicación para jóvenes (12-13 años):
         - Un "raycast" es como lanzar una línea invisible hacia abajo para ver si choca
           con el suelo.
         - Si choca, significa que el enemigo está tocando el suelo y puede saltar.
         - Esto evita que salte en el aire o haga cosas raras cuando está cayendo.
        */
    ...
    private void TryJumpIfNeeded(Vector3 toPlayer)
    {
        ...
        /*
         Explicación para jóvenes (12-13 años):
         - Se llama cuando el enemigo camina hacia el jugador.
         - Primero comprueba si puede saltar (tiempo, está en suelo, no saltó hace poco).
         - Si el jugador está por encima, salta para alcanzarlo.
         - Si hay una pared u obstáculo justo delante, también salta para sortearla.
         - Es como cuando tú saltas para subir una piedra en el camino.
        */
        if (rb == null) return;
    ...
    private IEnumerator ShootBurstRoutine()
    {
        ...
        /*
         Explicación para jóvenes (12-13 años):
         - "Ráfaga" significa que dispara varias balas seguidas, una detrás de otra.
         - Este método usa una "corrutina" (una función que puede esperar sin bloquear
           todo el juego) para pausar entre cada disparo.
         - Mientras dispara, el enemigo se detiene para apuntar mejor.
         - Al terminar la ráfaga, espera un tiempo largo (cooldown) antes de volver a disparar.
        */
        isShooting = true;
    ...
    private void FireBullet()
    {
        ...
        /*
         Explicación para jóvenes (12-13 años):
         - Crea una copia de la bala (prefab) en la posición del enemigo.
         - Calcula hacia dónde debe ir la bala (hacia el jugador) y le pone velocidad.
         - También asigna un daño y hace que la bala no choque con quien la disparó.
         - Si hay un sonido de disparo, lo reproduce en la posición del enemigo.
        */
    ...
    public void TakeDamage(int damage)
    {
        ...
        /*
         Explicación para jóvenes (12-13 años):
         - Cuando la bala (u otro ataque) golpea al enemigo, se llama a esta función.
         - Resta puntos de vida según el daño recibido.
         - Si la vida llega a 0 o menos, llama a Die() para manejar la muerte.
         - También intenta reproducir la animación de que le duele.
        */
        if (isDead) return;
    ...
    private void Die()
    {
        ...
        /*
         Explicación para jóvenes (12-13 años):
         - Marca que está muerto para que no haga nada más.
         - Hace que el cuerpo deje de moverse y que sus colisiones se apaguen (no molesta
           al jugador ni a otros enemigos).
         - Crea objetos de loot (si toca) según probabilidades, para que el jugador pueda
           recogerlos después.
         - Reproduce la animación de muerte y destruye el objeto pasado un ratito.
         - En el editor reproduce un sonido especial si existe.
        */
        if (isDead) return;
        isDead = true;

        // Drops: recorrer la tabla de loot y tirar según probabilidad
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        /*
         Explicación para jóvenes (12-13 años):
         - Si el jugador toca al enemigo (por ejemplo, choca con él), aquí se maneja
           que el jugador reciba daño y un empujón.
         - Antes de aplicar daño, respeta si el jugador está muerto, en modo admin o
           si es invulnerable (así no le hace daño cuando no debe).
         - Calcula la dirección del empujón para que el jugador salga volando ligeramente.
        */
        var player = collision.GetComponent<RedMovement>();
        // Respetar invulnerabilidad / modo admin de Red: no aplicar daño ni knockback.
        if (player != null && player.IsAlive && !isDead && !player.IsInvulnerable && !player.IsAdminModeActive)
        {
